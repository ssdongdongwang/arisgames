...
#warning remove
    ...
    int playerId = [AppModel sharedAppModel].playerId;
...
- (void)storeGame:(Game*)game completion:(store_locally_block_t)block {
    ...
    NSString *str = [NSString stringWithFormat:@"action=get_all&id=%d&player_id=%d", _gameId, [AppModel sharedAppModel].playerId];
...
- (void)storeItems:(NSArray *)itemsDictionary game:(MGame *)game medias:(NSMutableArray*)medias {
    ...
    for (NSDictionary *itemDictionary in itemsDictionary) {
        ...
        if (mitem && [mitem.itemId intValue] == itemId) {
            ...
            Item *item = [[AppServices sharedAppServices] parseItemFromDictionary:itemDictionary];
            ...
            newMItem.kind = [NSNumber numberWithInt:item.kind];
...
- (void)storeNpcs:(NSArray *)npcsDictionary game:(MGame *)game medias:(NSMutableArray*)medias {
    ...
    for (NSDictionary *npcDictionary in npcsDictionary) {
        ...
        if (mnpc && [mnpc.npcId intValue] == npcId) {
            ...
            Npc *npc = [[AppServices sharedAppServices] parseNpcFromDictionary:npcDictionary];
...
- (void)storeLocations:(NSArray *)locationsDictionary game:(MGame *)game player:(MPlayer*)player medias:(NSMutableArray *)medias{
    ...
    for (NSDictionary *locationDictionary in locationsDictionary) {
        ...
        Location *location = [[AppServices sharedAppServices] parseLocationFromDictionary:locationDictionary];
        if (mlocation && [mlocation.locationId intValue] == locationId) {
            mlocation.error = [NSNumber numberWithDouble:location.error];
            ...
            mNewLocation.error = [NSNumber numberWithDouble:location.error];
            mNewLocation.hidden = [NSNumber numberWithBool:location.hidden];
            mNewLocation.itemQty = [NSNumber numberWithInt:location.qty];
            mNewLocation.latitude = [NSNumber numberWithDouble:location.location.coordinate.latitude];
            mNewLocation.longitude = [NSNumber numberWithDouble:location.location.coordinate.longitude];
            mNewLocation.name = location.name;
            mNewLocation.type = location.objectType;
            mNewLocation.typeId = [NSNumber numberWithInt:location.objectId];
            mNewLocation.game = game;
            mNewLocation.player = player;
            int iconMediaId = [[locationDictionary objectForKey:@"icon_media_id"] intValue];
            if (iconMediaId) {
                MMedia *media = [self fetchForEntityName:@"Media" predicate:[NSPredicate predicateWithFormat:@"mediaId = %d", location.iconMediaId]];
...
- (JSONResult*)tabBarItemsForGame:(MGame*)game {
