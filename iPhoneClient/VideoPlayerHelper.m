...
- (BOOL)load:(NSString*)filename playImmediately:(BOOL)playOnTextureImmediately fromPosition:(float)seekPosition
{
    ...
    if (NOT_READY != mediaState && ERROR != mediaState) {
        ...
        const char* remoteURL = strstr([filename cStringUsingEncoding:NSASCIIStringEncoding], "://");
        
        // For on texture rendering, we need a local file
        if (NULL == remoteURL) {
            localFile = YES;
            
            // Play media from this app's Resources folder
            NSString* resourcesFolder = [[NSBundle mainBundle] resourcePath];
            NSString* fullPath = [resourcesFolder stringByAppendingPathComponent:filename];
...
- (BOOL)play:(BOOL)fullscreen fromPosition:(float)seekPosition
{
    ...
    if ((PLAYING != mediaState || playerType != requestedPlayerType) && NOT_READY > mediaState) {
        if (PLAYER_TYPE_NATIVE == requestedPlayerType) {
            ...
            QRScannerViewController* appDelegate = [[UIApplication sharedApplication] delegate];
...
- (GLuint)updateVideoData
{
    ...
    if (PLAYING == mediaState && PLAYER_TYPE_ON_TEXTURE == playerType) {
        ...
        if (NULL != pixelBufferBaseAddress) {
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, videoSize.width, videoSize.height, 0, GL_BGRA_EXT, GL_UNSIGNED_BYTE, pixelBufferBaseAddress);
