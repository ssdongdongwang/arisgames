...
{	
  ...
  /**
   * Delete most games
   * Used during table transfer to bring total database size down
   * @returns returnCode = 0 on success
   */	
  public function deleteMostGames()
  {
    $query = "SELECT * FROM games ORDER BY game_id";
    $rs = mysql_query($query);
    for($i = 0; $i< 500; $i++) 
    {
      if($i != 159 && $i != 2625 && $i != 3795 && $i != 344){
  ...
  public function deleteGame($intGameId)
  {
    ...
    //Delete Fountains
    $query = "DELETE FROM fountains WHERE game_id = '{$intGameId}'";
    NetDebug::trace($query);
    @mysql_query($query);
    if (mysql_error()) return new returnData(3, NULL, 'SQL Error');

    //Delete the Game Comments
    $query = "DELETE FROM game_comments WHERE game_id = '{$intGameId}'";
    NetDebug::trace($query);
    @mysql_query($query);
    if (mysql_error()) return new returnData(3, NULL, 'SQL Error');	

    //Delete the Game Tags
    $query = "DELETE FROM game_tags WHERE game_id = '{$intGameId}'";
    NetDebug::trace($query);
    @mysql_query($query);
    if (mysql_error()) return new returnData(3, NULL, 'SQL Error');	
    ...
    //Delete the Spawnables
    $query = "DELETE FROM spawnables WHERE game_id = '{$intGameId}'";
    NetDebug::trace($query);
    @mysql_query($query);
    if (mysql_error()) return new returnData(3, NULL, 'SQL Error');	
  ...
  public function addNoteTagToGame($gameId, $tag)
  {
