...
function generatePlayerLocations()
{
  ...
  while ($row = mysql_fetch_object($result))
    echo 'new google.maps.Marker({ position: new google.maps.LatLng(' . $row->latitude . ',' . $row->longitude . '), map: map, icon: \'map_icons/player_alpha_100.png\' });' . "\n";


  $interval = ' AND updated BETWEEN DATE_SUB(NOW(), INTERVAL 1 DAY) AND NOW()';
  $result = mysql_query($query.$interval);
  while ($row = mysql_fetch_object($result))
    echo 'new google.maps.Marker({ position: new google.maps.LatLng(' . $row->latitude . ',' . $row->longitude . '), map: map, icon: \'map_icons/player_alpha_66.png\' });' . "\n";

/*
  $interval = ' AND updated BETWEEN DATE_SUB(NOW(), INTERVAL 1 WEEK) AND NOW()';
  $result = mysql_query($query.$interval);
  while ($row = mysql_fetch_object($result))
    echo 'new google.maps.Marker({ position: new google.maps.LatLng(' . $row->latitude . ',' . $row->longitude . '), map: map, icon: \'map_icons/player_alpha_33.png\' });' . "\n";
*/
}
...
function generateGameLocations()
{
  ...
  $result = mysql_query($query.$interval);
  while ($row = mysql_fetch_object($result))
    echo 'new google.maps.Marker({ position: new google.maps.LatLng(' . $row->latitude . ',' . $row->longitude . '), map: map, icon: \'map_icons/game_alpha_100.png\' });' . "\n";


  $interval = ' AND games.created BETWEEN DATE_SUB(NOW(), INTERVAL 1 WEEK) AND NOW() GROUP BY games.game_id';
  $result = mysql_query($query.$interval);
  while ($row = mysql_fetch_object($result))
    echo 'new google.maps.Marker({ position: new google.maps.LatLng(' . $row->latitude . ',' . $row->longitude . '), map: map, icon: \'map_icons/game_alpha_66.png\' });' . "\n";

  $interval = ' AND games.created BETWEEN DATE_SUB(NOW(), INTERVAL 1 MONTH) AND NOW() GROUP BY games.game_id';
  $result = mysql_query($query.$interval);
  while ($row = mysql_fetch_object($result))
    echo 'new google.maps.Marker({ position: new google.maps.LatLng(' . $row->latitude . ',' . $row->longitude . '), map: map, icon: \'map_icons/game_alpha_33.png\' });' . "\n";
