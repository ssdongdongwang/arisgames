<?php
	...
	if ($newvals["remove_if_event_id"] == 'ADD') {	
		//insert a new item
		$new_id = new_event("New event to remove location " . $this->rec);
		
		//store new id here, replacing 'add'
		$query = "UPDATE $this->tb SET remove_if_event_id = '{$new_id}' WHERE location_id = '{$this->rec}'";
	...
	if ($newvals["require_event_id"] == 'ADD') {	
		//insert a new item
		$new_id = new_event("New required event by location " . $this->rec);
		
		//store new id here, replacing 'add'
		$query = "UPDATE $this->tb SET require_event_id = '{$new_id}' WHERE location_id = '{$this->rec}'";
		mysql_query($query); //for add
